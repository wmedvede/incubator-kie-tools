...
// hpaExistsForDeployment returns true if an HorizontalPodAutoscaler targeting the deployment exists in the current namespace.
func hpaExistsForDeployment(ctx context.Context, c client.Client, namespace string, name string) (bool, error) {
	hpa, err := findHPAForDeployment(ctx, c, namespace, name)
	return hpa != nil, err
}
