...
execSync(
  `${activateCmd} && \
  python3 ${sonataflowImageCommonDir}/resources/scripts/versions_manager.py --bump-to ${env.oslOperatorBundleImage.buildTag} --source-folder ./resources`,
  { stdio: "inherit" }
);
...
const newYamlFilePath = path.join(resourcesPath, `${env.oslOperatorBundleImage.name}-image.yaml`);
