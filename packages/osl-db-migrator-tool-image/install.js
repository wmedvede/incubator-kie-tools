...
const oslImagesModulesDir = path.dirname(require.resolve("@osl/osl-images-modules/package.json"));
...
execSync(
  `${activateCmd} && \
  python3 ${sonataflowImageCommonDir}/resources/scripts/versions_manager.py --bump-to ${env.oslDbMigratorToolImage.buildTag} --source-folder ./resources`,
  { stdio: "inherit" }
);
...
const newYamlFilePath = path.join(resourcesPath, `${env.oslDbMigratorToolImage.name}-image.yaml`);

fs.renameSync(originalYamlPath, newYamlFilePath);

execSync(
  `${activateCmd} && \
  python3 ${oslImagesModulesDir}/resources/scripts/update_osbs_branch.py --version ${env.osl.branchName} --file ${newYamlFilePath}`,
  { stdio: "inherit" }
);

const imageUrl = `${env.oslDbMigratorToolImage.account}/${env.oslDbMigratorToolImage.name}`;
